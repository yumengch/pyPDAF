...
class ObsA:
    ...
    def init_dim(self, step:int, dim_obs:int) -> int:
        ...
        if pyPDAF.PDAF.get_local_type() > 1:
            dim_p = self.model.nx*self.model.ny
            coords_p = np.zeros((2, dim_p), order='F')
            offset = self.pe.mype_filter*self.model.nx_p
            coords_p[0] = np.tile(np.arange(self.model.nx_p) + \
                                offset, self.model.ny_p)
            coords_p[1] = np.repeat(np.arange(self.model.ny_p),
                                    self.model.nx_p)
