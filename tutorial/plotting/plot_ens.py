...
def read_ensemble():
    ...
    for i in range(int(ntime/2)):
        for j in range(nens):
            field = np.loadtxt("ens_{0:02d}_step{1:02d}_for.txt".format(j+1, 2*(i+1)),delimiter=';')
            ens[2*i, ..., j] = np.loadtxt("ens_{0:02d}_step{1:02d}_for.txt".format(j+1, 2*(i+1)),delimiter=';')
            ens[2*i+1, ..., j] = np.loadtxt("ens_{0:02d}_step{1:02d}_ana.txt".format(j+1, 2*(i+1)),delimiter=';')
    return ens
