...
def _unbiased_moments_from_summed_residuals(int  dim_ens, int  dim_p, 
    int  kmax, double [::1,:] sum_expo_resid, double [::1,:] moments):
    """Checking the corresponding PDAF documentation in https://pdaf.awi.de
    For internal subroutines checking corresponding PDAF comments.
    ...
    sum_expo_resid : ndarray[np.float64, ndim=2]
    ...
    moments : ndarray[np.float64, ndim=2]
        ...
        Array shape: (dim_p, kmax)
    """
    cdef cnp.ndarray[cnp.float64_t, ndim=2, mode="fortran", negative_indices=False, cast=False] moments_np = np.asarray(moments, dtype=np.float64, order="F")
    with nogil:
        c__pdaf_unbiased_moments_from_summed_residuals(&dim_ens, &dim_p, 
                                                       &kmax, 
                                                       &sum_expo_resid[0,0], 
                                                       &moments[0,0])
...
def _biased_moments_from_summed_residuals(int  dim_ens, int  dim_p, 
    int  kmax, double [::1,:] sum_expo_resid, double [::1,:] moments):
    """Checking the corresponding PDAF documentation in https://pdaf.awi.de
    For internal subroutines checking corresponding PDAF comments.
    ...
    cdef cnp.ndarray[cnp.float64_t, ndim=2, mode="fortran", negative_indices=False, cast=False] moments_np = np.asarray(moments, dtype=np.float64, order="F")
    with nogil:
        c__pdaf_biased_moments_from_summed_residuals(&dim_ens, &dim_p, 
                                                     &kmax, 
                                                     &sum_expo_resid[0,0], 
                                                     &moments[0,0])
