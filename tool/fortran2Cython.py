...
def writeUserCalls(filename, UserFuncInfo):
    with open(filename, 'w') as f:
        ...
        s += '# put this somewhere but before calling the asserts\n'
        s += 'sys_excepthook = sys.excepthook\n'
        s += 'def mpi_excepthook(type, value, traceback): \n'
        s += '    sys_excepthook(type, value, traceback) \n'
        s += '    if MPI.COMM_WORLD.size > 1:\n'
        s += '        MPI.COMM_WORLD.Abort(1) \n'
        s += 'sys.excepthook = mpi_excepthook\n\n\n'
        f.write(s)
        for routine in UserFuncInfo:
            name = routine.pop('name')
